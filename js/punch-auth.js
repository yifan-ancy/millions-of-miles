(function () {
  ...
  if (window.Session) {
    if (!window.Session.ensureSignedIn()) return;
    window.Session.mountUserBadge();
  }
  ...
  function savePlans() {
    persistLocalPlans();
    if (!window.Session || !window.Session.getToken()) return;
    clearTimeout(remoteSaveTimer);
    remoteSaveTimer = setTimeout(function () {
      window.Session.savePlans(state.plans).catch(function () {});
    }, 180);
  }
  ...
  function openEditor(entry) {
    ...
    reachBtn.onclick = function () {
      ensurePlan(entry);
      if (state.plans[item.name].reachedAt) state.plans[item.name].reachedAt = null;
      else state.plans[item.name].reachedAt = Date.now();
  ...
  function tryAddFromQuery() {
    ...
    openEditor(entry);
    if (history.replaceState) history.replaceState({}, "", "punch.html");
    return true;
  }
  ...
  async function hydratePlans() {
    state.plans = loadLocalPlans();
    renderAll();
    if (!window.Session || !window.Session.getToken()) return;
    try {
      state.plans = await window.Session.loadPlans();
      persistLocalPlans();
      renderAll();
    } catch (error) {}
  }
